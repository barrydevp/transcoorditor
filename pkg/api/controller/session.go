...
func (ctrl *Controller) PutSessionById(c *fiber.Ctx) error {
	...
	if session == nil {
		return SendResponse(c, util.Errorf("session not found"))
	}
...
func (ctrl *Controller) CommitSession(c *fiber.Ctx) error {
	...
	if err != nil {
		return SendResponse(c, util.Errorf("cannot commit session, %w", err))
	}
...
func (ctrl *Controller) AbortSession(c *fiber.Ctx) error {
	...
	if err != nil {
		return SendResponse(c, util.Errorf("cannot abort session, %w", err))
	}
